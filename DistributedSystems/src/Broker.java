...
public class Broker extends Thread {
    ...
    List<Consumer> enrolledConsumers = new ArrayList<Consumer>();
    List<Publisher> enrolledPublishers = new ArrayList<Publisher>();
    ...
    private Long hashCode(String input) throws NoSuchAlgorithmException {
        ...
        // GENIKA, to messageDigest pairnei to input ws pinaka apo byte kai epistrefei
        // ena MD5 hash instance
        byte[] messageDigest = md.digest(input.getBytes());

        // Convert byte array into signum representation
        // Translates the sign-magnitude representation of a BigInteger into a
        // BigInteger (signed to big integer)
    ...
    private void hashOfBrokers() throws NoSuchAlgorithmException {
        ...
        for (Broker br : brokers) {
            ...
            // foreach broker found in the list, split its ip string value and return it
            // inside "parts" array
            // kathe thesi tou pinaka xwrizetai apo ta kommatia pou kathorizontai apo to
            // regular expression
            String[] parts = br.getIp().split("\\.");
            String i = "";
            // iterate the array that has just been generated, that includes the ip parts of
            // the whole ip of specific broker
            // adds to string "i" the value of the positions of parts array, so a big string
            // consisted of the whole ip of
             ...
             * i=i + parts[j];
             * }
             * //in is an integer that is the sum of the i (converted to integer) and the
             * port of that broker
             * int in = Integer.parseInt(i) + br.getPort();
             * String input = Integer.toString(in);
             */
            // i = sum of the characters of the ip, for ex: ip = 192.168.10.23 =>
            // i=1921681023
            String result = "";
            for (int j = 0; j < parts.length; j++) {
            ...
            // an h ip tou broker pou ektelei thn parousa synarthsh einai h idia me thn ip
            // apo thn synoliki lista twn
            // brokers, epishs an tautoxrona to port tou parontos broker einai to idio me
            // ayto pou eksetazetai, tote vale
            // ws hash timh aytou touu broker to apotelesma tou hash pou tha vgalei h
            // synarthsh me to sygkekrimeno input
            if ((this.getIp().equals(br.getIp())) && (this.getPort() == br.getPort())) {
                ...
                // alliws vale ston broker pou exoume ayth th stigmh sto iteration, to
                // apotelesma tou hash code tou input
    ...
    static class BrokerHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                 ...
                 * File("C:\\Users\\Pelagia\\OneDrive - aueb.gr\\Desktop\\mediaFile\\Red_Kitten_01.jpg"
                 * );
                 * try{
